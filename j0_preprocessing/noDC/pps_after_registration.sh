...
do
	...
	do	
		...
		do	
			...
			CMD="rsbatch -A subject={} -A measurementnumber=$m -A TR=$t -A run=$r -j $job -s 1-7,9-100"
			CMD1="rsbatch -A subject={} -A measurementnumber=$m -A TR=$t -A run=$r -j $job --threads=$threads -s 1-14"
			echo "$subjects" | parallel -j+0 --eta --joblog pps.log -S$worker $CMD 
			echo "$subjects" | parallel -j+0 --eta --joblog pps.log -S$worker $CMD1 
			#echo "$CMD"
