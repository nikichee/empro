function [allbet, allbet_psc, allbet_mean] = extractbetas_allsubsfromfl_sphere_wmcsf_coofromall(TR)
...
% if strcmp(TR, '1.4')
%     [voxelcoo, ~, ~]=getclustercoordinates_all_eedt_odt();
% elseif strcmp(TR, '0.7')
%     [voxelcoo, ~, ~]=getclustercoordinates_all_eedt_odt_TR07();
%     zusatz='_coo0.7';
% else
%     disp('wrong TR? please enter string of 0.7 or 1.4')
%     return
% end
...
%             TR = TRs{t};
            ...
            betas(bi+4) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0022.nii')]};
            betas(bi+5) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0023.nii')]};
            betas(bi+6) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0024.nii')]};
            betas(bi+7) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0043.nii')]};
            betas(bi+8) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0044.nii')]};
            betas(bi+9) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0045.nii')]};
            
            betameans(bi+1) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0064.nii')]};
            betameans(bi+2) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0064.nii')]};
            betameans(bi+3) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0064.nii')]};
            betameans(bi+4) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0065.nii')]};
            betameans(bi+5) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0065.nii')]};
            betameans(bi+6) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0065.nii')]};
            betameans(bi+7) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0066.nii')]};
            betameans(bi+8) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0066.nii')]};
            betameans(bi+9) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0066.nii')]};
%             betas(bi+1) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0001.nii')]};
%             betas(bi+2) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0002.nii')]};
%             betas(bi+3) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0003.nii')]};
...
%[voxelindices, mnicoo, clusternames] = getclustercoordinates_all_eedt_odt();
save(fullfile(datadir, ['parameterestimates_sphere_coofromall_' num2str(radius) 'vx_' TR '.mat']), 'allbet', 'allbet_psc', 'allbet_mean', 'voxelindices', 'mnicoo', 'clusternames');
