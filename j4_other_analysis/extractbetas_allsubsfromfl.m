function [allbet, allbet_psc, allbet_mean] = extractbetas_allsubsfromfl(TR)
...
for s=1:size(subs,1)
    ...
    for mi=1:size(ms,2)
        ...
        for t=1:size(TRs,2)
            ...
            betameans(bi+1) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0028.nii') ',1']};
            betameans(bi+2) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0028.nii') ',1']};
            betameans(bi+3) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0028.nii') ',1']};
            betameans(bi+4) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0029.nii') ',1']};
            betameans(bi+5) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0029.nii') ',1']};
            betameans(bi+6) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0029.nii') ',1']};
            betameans(bi+7) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0030.nii') ',1']};
            betameans(bi+8) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0030.nii') ',1']};
            betameans(bi+9) = {[fullfile(firstleveldir,meas,TR, subs(s).name, '/beta_0030.nii') ',1']};
            bi=bi+9;
        end
    end
%     disp(betas)
