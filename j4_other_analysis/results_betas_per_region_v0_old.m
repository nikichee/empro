...
regions(ind).name = 'Parahippocampal_Gyr_AND_Amygdala_R';
...
regions(ind).name = 'Parahippocampal_Gyr_AND_Amygdala_L';
...
regions(ind).name = 'Fusiform_Gyr1';
...
regions(ind).name = 'Inferior_Frontal_Gyr_DLPFC';
...
regions(ind).name = 'Middle_Temporal_Gyr2';
...
regions(ind).name = 'Middle_Temporal_Gyr3';
regions(ind).coords = [   49.5000
  -13.0000
  -16.0000
    ];
...
regions(ind).name = 'Middle_Frontal_Gyr1';
...
regions(ind).name = 'Middle_Frontal_Gyr2';
...
for k=1:size(regions(1).results,2)
    ...
    C(1,k+1)={regions(1).results(k).name};
    for l=1:size(regions,2)        
        C(l+1,k+1)={regions(l).results(k).val};
    end
end
for l=1:size(regions,2)     
C{l+1,1}={regions(l).name};%{regions(l).name};
end
