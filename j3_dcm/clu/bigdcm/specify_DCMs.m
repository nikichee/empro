...
subs = importdata('/z/fmri/data/empro15/analysis/edt/3_dcm/tmp_clu/mdd.rem_current.txt');
...
for s=1:length(subs);
    for m=1:1
        for dirs =1:3            
            ...
            thisdir = fullfile(DCMdir, ['m000' num2str(dirs)]); % ['/z/fmri/data/empro15/analysis/edt/3_dcm/'m' num2str(m) '/' TRs{t} '/' subs{s} '/'];
            
            thisDCM=fullfile(thisdir, ['DCM_' subs{s} '.mat']);
            copyfile(fullfile(thisdir, template), thisDCM); % copy template
            
            VOI_dir = ['/z/fmri/data/clu12/analysis/edt/analysis_spm/fl_preprocv3_wmcsf/m1/mdd.rem/' subs{s}];
